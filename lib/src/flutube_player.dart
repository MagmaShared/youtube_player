...
class FluTubeState extends State<FluTube>{
  ...
  initState() {
    ...
    if(_isPlaylist) {
      _initialize(_currentUrl = (widget._videourls as List<String>)[0]); // Play the very first video of the playlist
    } else {
      _initialize(_currentUrl = widget._videourls as String);
    }

  }

  Future<void> _initialize(String _url) {

    debugPrint('INITIALIZE: $_url');

    _lastUrl = _url;
    return _fetchVideoURL(_url).then((url) {
  ...
  _startListener() {
    if(isPlaying && (videoController?.value?.initialized ?? false))
      widget.onVideoStart();
  }
  ...
  _errorListener() {
    if (videoController?.value?.hasError == false ?? false) return;
    if (videoController?.value?.errorDescription?.contains("code: 403") ?? false) _initialize(_lastUrl);
  }
  ...
  Future<void> _updateVideoUrl() async {
    ...
    if (_currentUrl != null && widget._videourls != _currentUrl){
      _currentUrl = widget._videourls;
      debugPrint('*** UPDATING NEW VIDEO URL: $_currentUrl ***');

      await videoController.pause();
      if (videoController?.value?.duration != null) await chewieController?.seekTo(Duration());
      ...
      if(_isPlaylist) {
        await _initialize((widget._videourls as List<String>)[0]); // Play the very first video of the playlist
      } else {
        await _initialize(widget._videourls as String);
      }

      setState(() { 
        if (widget.autoPlay) videoController.play();
        debugPrint('*** NEW VIDEO INITIALIZED ***');
      });
    }
  }
